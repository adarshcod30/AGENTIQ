...
describe('runAssessment (stubbed pipeline)', () => {
  ...
  it('never starts a cloned (untrusted) project, running discovery and static scans only', async () => {
    const project = await Project.create({ userId, name: 'Cloned', workspaceRoot: VULN, trusted: false });
    const created = await createAssessment({ userId, projectId: project._id, schedule: false });
    ...
    const deps = {
      ...stubDeps(),
      ensureApp: async () => { throw new Error('ensureApp must never run for an untrusted project'); },
    ...
    expect(done.state).toBe(ASSESS_STATE.COMPLETE);
    expect(done.baseUrl).toBeNull(); // nothing was started
    expect(functionalCalls).toBe(0); // no scripts ran, so no live tests
    expect(done.endpoints.every((e) => e.status === 'skipped')).toBe(true);
    expect(scanArgs.runStatic).toBe(true); // the cloned source is still statically scanned
    expect(done.security.notes.some((n) => /untrusted/i.test(n))).toBe(true);
  });
